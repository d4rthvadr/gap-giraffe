...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const modelSelect = document.getElementById('model-name') as HTMLSelectElement;
  ...
  async function saveConfiguration(): Promise<void> {
    try {
      ...
      try {
        ...
        const errorMsg = error instanceof Error ? error.message : 'Failed to validate API key';
        showStatus(`Configuration saved, but validation failed: ${errorMsg}`, 'error');
